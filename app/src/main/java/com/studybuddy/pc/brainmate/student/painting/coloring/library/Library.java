package com.studybuddy.pc.brainmate.student.painting.coloring.library;
...
import com.studybuddy.pc.brainmate.student.painting.util.InputStreamProvider;
...
public class Library {
    ...
    // of the color picker on the coloring activity

    private Library(Context context1) {
        this.context1 = context1;
        libraryFileRootFolder = context1.getString(R.string.library_root_folder) + File.separator;

      String libraryFilePath = libraryFileRootFolder + context1.getString(R.string.library_file);
      Log.d("libraryFilePath",libraryFilePath);
      ...
      try
      {
            InputStream is = context1.getAssets().open(libraryFilePath);
      ...
      catch (IOException e)
      {
        ...
        try {
            ...
            Log.d("booksbooks", String.valueOf(books));
    ...
    private InputStreamProvider getInputStreamProviderForAssetPath(final String pathName) {
        return new InputStreamProvider() {
            ...
            public InputStream getStream() {
                InputStream is;
                try {
                    is = context1.getAssets().open(pathName);
                } catch (IOException e) {
                    throw new RuntimeException(e);
                }
                return is;
